<x-app-layout>
    <div class="flex flex-col min-h-screen overflow-hidden md:flex-row">
        ...
        <div class="flex-1 overflow-y-auto bg-gray-100 dark:bg-gray-900">
            <main class="p-4 mx-auto space-y-6 sm:p-6 lg:p-8">
                ...
                <x-search-filter-admin action="{{ route('admin.tahun_ajaran.index') }}" :search="request('search')"
                    :sortOptions="[
                                    ...
                                    <tr class="transition-colors hover:bg-gray-50 dark:hover:bg-gray-700">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900 dark:text-white">
                                                {{ $tahun_ajaran->tahun_ajaran ?? '-' }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span
                                                class="inline-flex px-2 text-xs font-semibold leading-5 rounded-full
                                        {{ $tahun_ajaran->status ? 'text-green-800 bg-green-100 dark:text-green-200 dark:bg-green-900' : 'text-red-800 bg-red-100 dark:text-red-200 dark:bg-red-900' }}">
                                                {{ $tahun_ajaran->status_label }}
                                            </span>
                                        </td>
                                        <td
                                            ...
                                            <div class="flex justify-center gap-2">
                                                ...
                                                <form id="delete-form-{{ $tahun_ajaran->id }}"
                                                    action="{{ route('admin.tahun_ajaran.destroy', $tahun_ajaran) }}"
                                        ...
                                        <td colspan="100%" class="px-6 py-10 text-center">
                                            <div class="flex flex-col items-center justify-center p-6 text-center">
                                                ...
                                                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                                    ...
                                                    @else
                                                        Belum ada tahun ajaran yang ditambahkan ke dalam sistem
                                                    @endif
                                                </p>
                                                <div class="mt-6">
                    ...
                    <div class="px-4 py-3 bg-white border-t border-gray-200 sm:px-6">
                        <div class="flex flex-col items-center justify-between gap-4 sm:flex-row">
                            ...
                            <div class="flex flex-col items-center gap-2 text-sm text-gray-700 sm:flex-row">
                                <span>
                                    Menampilkan
                                    <span class="font-medium">{{ $tahunAjaran->firstItem() ?? 0 }}</span>
                                    sampai
                                    <span class="font-medium">{{ $tahunAjaran->lastItem() ?? 0 }}</span>
                                    dari
                                    <span class="font-medium">{{ $tahunAjaran->total() }}</span>
                                    hasil
                                </span>
                                <span class="hidden text-gray-400 sm:inline">•</span>
                                <span>
                                    Halaman
                                    <span class="font-medium">{{ $tahunAjaran->currentPage() }}</span>
                                    dari
                                    <span class="font-medium">{{ $tahunAjaran->lastPage() }}</span>
                                </span>
                            </div>
                        ...
                        <div class="mt-3 sm:hidden">
                            <select onchange="window.location.href=this.value"
                                ...
                                @for ($i = 1; $i <= $tahunAjaran->lastPage(); $i++)
                                    <option value="{{ $tahunAjaran->url($i) }}"
                                        {{ $i == $tahunAjaran->currentPage() ? 'selected' : '' }}>
                                        Halaman {{ $i }}
                                    </option>
                                @endfor
                            </select>
    ...
    <x-delete-modal title="Konfirmasi Hapus"
        description="Apakah Anda yakin ingin menghapus data tahun ajaran ini? Tindakan ini tidak dapat dibatalkan dan akan menghapus semua data terkait tahun ajaran tersebut." />
</x-app-layout>
