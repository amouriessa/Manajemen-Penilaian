<x-app-layout>
    <div class="flex flex-col min-h-screen overflow-hidden md:flex-row">
        ...
        <div class="flex-1 overflow-y-auto bg-gray-100 dark:bg-gray-900">
            <main class="p-4 mx-auto space-y-6 sm:p-6 lg:p-8">
                ...
                <x-search-filter-admin action="{{ route('admin.tahun_angkatan.index') }}" :search="request('search')" :sortOptions="[
                    'status_desc' => 'Status Aktif',
                    'status_asc' => 'Status Tidak Aktif',
                    'newest' => 'Terbaru',
                    'oldest' => 'Terlama',
                ]" />
                                ...
                                <tr class="transition-colors hover:bg-gray-50 dark:hover:bg-gray-700">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="inline-flex px-2 text-xs font-semibold leading-5 rounded-full
                                            {{ $tahun_angkatan->status ? 'text-green-800 bg-green-100 dark:text-green-200 dark:bg-green-900' : 'text-red-800 bg-red-100 dark:text-red-200 dark:bg-red-900' }}">
                                            {{ $tahun_angkatan->status_label }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 text-sm font-medium text-center text-gray-900 dark:text-white whitespace-nowrap">
                                        <div class="flex justify-center gap-2">
                                            ...
                                            <form id="delete-form-{{ $tahun_angkatan->id }}" action="{{ route('admin.tahun_angkatan.destroy', $tahun_angkatan) }}" method="POST" class="hidden">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </div>
                                ...
                                <tr>
                                    <td colspan="100%" class="px-6 py-10 text-center">
                                        <div class="flex flex-col items-center justify-center p-6 text-center">
                                            ...
                                            <h3 class="mt-4 text-lg font-medium text-gray-900 dark:text-white">Tidak ada tahun angkatan</h3>
                                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                                @if (request('search'))
                                                    Tidak ada hasil untuk pencarian "{{ request('search') }}"
                                                @else
                                                    Belum ada tahun angkatan yang ditambahkan ke dalam sistem
                                                @endif
                                            </p>
                                            <div class="mt-6">
                    ...
                    <div class="px-4 py-3 bg-white border-t border-gray-200 sm:px-6">
                        <div class="flex flex-col items-center justify-between gap-4 sm:flex-row">
                            ...
                            <div class="flex flex-col items-center gap-2 text-sm text-gray-700 sm:flex-row">
                                <span>
                                    Menampilkan
                                    <span class="font-medium">{{ $tahunAngkatan->firstItem() ?? 0 }}</span>
                                    sampai
                                    <span class="font-medium">{{ $tahunAngkatan->lastItem() ?? 0 }}</span>
                                    dari
                                    <span class="font-medium">{{ $tahunAngkatan->total() }}</span>
                                    hasil
                                </span>
                                <span class="hidden text-gray-400 sm:inline">•</span>
                                <span>
                                    Halaman
                                    <span class="font-medium">{{ $tahunAngkatan->currentPage() }}</span>
                                    dari
                                    <span class="font-medium">{{ $tahunAngkatan->lastPage() }}</span>
                                </span>
                            </div>

                            <!-- Navigation Links -->
                            <div class="flex items-center space-x-2">
                                @if ($tahunAngkatan->onFirstPage())
                                    <span
                                ...
                                @else
                                    <a href="{{ $tahunAngkatan->previousPageUrl() }}"
                                ...
                                <div class="items-center hidden space-x-1 sm:flex">
                                    @foreach ($tahunAngkatan->getUrlRange(max(1, $tahunAngkatan->currentPage() - 2), min($tahunAngkatan->lastPage(), $tahunAngkatan->currentPage() + 2)) as $page => $url)
                                        @if ($page == $tahunAngkatan->currentPage())
                                            <span
                                ...
                                @if ($tahunAngkatan->hasMorePages())
                                    <a href="{{ $tahunAngkatan->nextPageUrl() }}"
                                        class="relative inline-flex items-center px-3 py-2 text-sm font-medium text-gray-700 transition-colors duration-200 bg-white border border-gray-300 rounded-md hover:bg-gray-50 hover:text-gray-900">
    ...
    <x-delete-modal title="Konfirmasi Hapus"
        description="Apakah Anda yakin ingin menghapus data tahun angkatan ini? Tindakan ini tidak dapat dibatalkan dan akan menghapus semua data terkait tahun angkatan tersebut." />
</x-app-layout>
