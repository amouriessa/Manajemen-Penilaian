<x-app-layout>
    <div class="flex flex-col min-h-screen overflow-hidden md:flex-row">
        ...
        <div class="flex-1 overflow-y-auto bg-gray-100 dark:bg-gray-900">
            <main class="p-4 mx-auto space-y-6 sm:p-6 lg:p-8">
                ...
                <x-search-filter-admin action="{{ route('admin.kelas_tahfidz.index') }}" :search="request('search')" :sortOptions="[
                    'name_asc' => 'Nama (A-Z)',
                    'name_desc' => 'Nama (Z-A)',
                    'newest' => 'Terbaru',
                    'oldest' => 'Terlama',
                            ...
                            <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                                ...
                                <tr class="transition-colors hover:bg-gray-50 dark:hover:bg-gray-700">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900 dark:text-white">{{ $kelas_tahfidz->tingkatan_label ?? '-' }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900 dark:text-white">{{ $kelas_tahfidz->nama ?? '-' }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900 dark:text-white">{{ $kelas_tahfidz->guru->user->name ?? '-' }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900 dark:text-white">{{ $kelas_tahfidz->total_siswa ?? '-' }}</div>
                                    </td>
                                    <td class="px-6 py-4 text-sm font-medium text-center text-gray-900 dark:text-white whitespace-nowrap">
                                        <div class="flex justify-center gap-2">
                                            ...
                                            <form id="delete-form-{{ $kelas_tahfidz->id }}" action="{{ route('admin.kelas_tahfidz.destroy', $kelas_tahfidz) }}" method="POST" class="hidden">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </div>
                                ...
                                <tr>
                                    <td colspan="100%" class="px-6 py-10 text-center">
                                        <div class="flex flex-col items-center justify-center p-6 text-center">
                                            ...
                                            <h3 class="mt-4 text-lg font-medium text-gray-900 dark:text-white">Tidak ada kelas</h3>
                                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                                @if (request('search'))
                                                    Tidak ada hasil untuk pencarian "{{ request('search') }}"
                                                @else
                                                    Belum ada kelas yang ditambahkan ke dalam sistem
                                                @endif
                                            </p>
                                            <div class="mt-6">
                                                <a href="{{ route('admin.kelas_tahfidz.create') }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-indigo-600 border border-transparent rounded-md shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                    ...
                    <div class="px-4 py-3 bg-white border-t border-gray-200 sm:px-6">
                        <div class="flex flex-col items-center justify-between gap-4 sm:flex-row">
                            ...
                            <div class="flex flex-col items-center gap-2 text-sm text-gray-700 sm:flex-row">
                                <span>
                                    Menampilkan
                                    <span class="font-medium">{{ $kelasTahfidz->firstItem() ?? 0 }}</span>
                                    sampai
                                    <span class="font-medium">{{ $kelasTahfidz->lastItem() ?? 0 }}</span>
                                    dari
                                    <span class="font-medium">{{ $kelasTahfidz->total() }}</span>
                                    hasil
                                </span>
                                <span class="hidden text-gray-400 sm:inline">•</span>
                                <span>
                                    Halaman
                                    <span class="font-medium">{{ $kelasTahfidz->currentPage() }}</span>
                                    dari
                                    <span class="font-medium">{{ $kelasTahfidz->lastPage() }}</span>
                                </span>
                            </div>

                            <!-- Navigation Links -->
                            <div class="flex items-center space-x-2">
                                @if ($kelasTahfidz->onFirstPage())
                                    <span
                                ...
                                @else
                                    <a href="{{ $kelasTahfidz->previousPageUrl() }}"
                                ...
                                <div class="items-center hidden space-x-1 sm:flex">
                                    @foreach ($kelasTahfidz->getUrlRange(max(1, $kelasTahfidz->currentPage() - 2), min($kelasTahfidz->lastPage(), $kelasTahfidz->currentPage() + 2)) as $page => $url)
                                        @if ($page == $kelasTahfidz->currentPage())
                                            <span
                                ...
                                @if ($kelasTahfidz->hasMorePages())
                                    <a href="{{ $kelasTahfidz->nextPageUrl() }}"
                                        class="relative inline-flex items-center px-3 py-2 text-sm font-medium text-gray-700 transition-colors duration-200 bg-white border border-gray-300 rounded-md hover:bg-gray-50 hover:text-gray-900">
                        ...
                        <div class="mt-3 sm:hidden">
                            <select onchange="window.location.href=this.value"
                                ...
                                @for ($i = 1; $i <= $kelasTahfidz->lastPage(); $i++)
                                    <option value="{{ $kelasTahfidz->url($i) }}"
                                        {{ $i == $kelasTahfidz->currentPage() ? 'selected' : '' }}>
                                        Halaman {{ $i }}
                                    </option>
                                @endfor
                            </select>
    ...
    <x-delete-modal title="Konfirmasi Hapus"
        description="Apakah Anda yakin ingin menghapus data kelas ini? Tindakan ini tidak dapat dibatalkan dan akan menghapus semua data terkait kelas tersebut." />
</x-app-layout>
