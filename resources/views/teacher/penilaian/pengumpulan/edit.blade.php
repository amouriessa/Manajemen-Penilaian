        ...
        <div class="{{ empty($sidebar) ? 'w-full' : 'flex-1' }} overflow-y-auto bg-gray-100 dark:bg-gray-900">
            <main class="w-full max-w-4xl p-4 mx-auto space-y-6 sm:p-6 lg:p-8">
                ...
                <div
                    ...
                    <form action="{{ route('teacher.penilaian.pengumpulan.update', $pengumpulan) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <input type="hidden" name="tugas_hafalan_id" value="{{ $penilaian->tugas_hafalan_id }}">
                        <input type="hidden" name="jenis_penilaian" value="pengumpulan">
                        <input type="hidden" name="jenis_hafalan" value="{{ $penilaian->jenis_hafalan }}">

                        <div class="grid grid-cols-1 gap-6 mb-6 sm:grid-cols-2 lg:grid-cols-3">
                            <div>
                                <label for="nilai_tajwid" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                                    Nilai Tajwid (0-100)
                                </label>
                                <input type="number" name="nilai_tajwid" id="nilai_tajwid" min="0" max="100"
                                    value="{{ old('nilai_tajwid', $penilaian->nilai_tajwid) }}"
                            ...
                            <div>
                                <label for="nilai_harakat" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                                    Nilai Harakat (0-100)
                                </label>
                                <input type="number" name="nilai_harakat" id="nilai_harakat" min="0" max="100"
                                    value="{{ old('nilai_harakat', $penilaian->nilai_harakat) }}"
                                    class="block w-full mt-1 text-sm border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                                    required>
                                @error('nilai_harakat')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            ...
                            <div>
                                <label for="nilai_makhraj" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                                    Nilai Makhraj (0-100)
                                </label>
                                <input type="number" name="nilai_makhraj" id="nilai_makhraj" min="0" max="100"
                                    value="{{ old('nilai_makhraj', $penilaian->nilai_makhraj) }}"
                                    class="block w-full mt-1 text-sm border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                                    required>
                                @error('nilai_makhraj')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="catatan" id="catatan" rows="4"
                                class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">{{ old('catatan', $penilaian->catatan) }}</textarea>
                            @error('catatan')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
