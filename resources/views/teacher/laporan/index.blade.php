    ...
    <div class="flex flex-col min-h-screen overflow-hidden md:flex-row">
        ...
        <div class="flex-1 overflow-y-auto">
            <main class="p-4 mx-auto space-y-6 sm:p-6 lg:p-8">
                ...
                {{-- Header --}}
                <div
                    class="flex flex-col gap-4 p-6 bg-white border-l-4 border-indigo-600 shadow-md rounded-xl dark:bg-gray-800 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                ...
                <div class="p-6 bg-white shadow-md rounded-xl dark:bg-gray-800">
                    ...
                    <form method="GET" action="{{ route('teacher.laporan.index') }}" id="form-laporan"
                        class="space-y-4">
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <select name="kelas_tahfidz_id" id="kelas_tahfidz_id"
                                    class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-100 focus:border-indigo-700 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                                    <option value="">Pilih Kelas</option>
                                    @foreach ($kelasList as $kelas)
                                        {{-- Tampilkan tingkatan + nama: misal "Kelas 7 - A" atau sesuai field yang ada --}}
                                        <option value="{{ $kelas->id }}"
                                            {{ request('kelas_tahfidz_id') == $kelas->id ? 'selected' : '' }}>
                                            {{ $kelas->tingkatan_label ? 'Kelas ' . $kelas->tingkatan_label . ' - ' . $kelas->nama : $kelas->nama }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                                ...
                                <div class="flex flex-wrap gap-4">
                                    <div class="flex-1 min-w-[200px]">
                                        ...
                                        <input type="date" name="dari_tanggal"
                                            class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-100 focus:border-indigo-700"
                ...
                @if ($selectedStudent)
                    ...
                    <div class="p-6 bg-white shadow-md rounded-xl dark:bg-gray-800">
                        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                            <div class="flex-1">
                                <h2 class="font-serif text-xl font-bold text-gray-800 dark:text-white">
                                    {{ $selectedStudent->user->name }}
                                </h2>
                                <p class="text-sm text-gray-600 dark:text-gray-400">{{ $selectedStudent->user->email }}
                                </p>
                                @if ($selectedStudent->riwayatKelas->isNotEmpty())
                                    @php
                                        $kelas = $selectedStudent->riwayatKelas->last()->kelasTahfidz ?? null;
                                    @endphp
                            ...
                            <div class="flex flex-wrap gap-3">
                                <a href="{{ route('teacher.laporan.cetak', request()->all()) }}" target="_blank"
                                    class="inline-flex items-center px-4 py-2 text-sm font-medium text-white transition-colors bg-green-600 rounded-lg hover:bg-[#638070] focus:ring-2 focus:ring-green-600 focus:ring-offset-2">
                    ...
                    <div class="bg-white shadow-md rounded-xl dark:bg-gray-800">
                        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                            <div class="flex items-center justify-between">
                                <div>
                                    <h3 class="text-lg font-semibold text-gray-800 dark:text-white">Riwayat Penilaian
                                    </h3>
                                    <p class="text-sm text-gray-600 dark:text-gray-400">
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead class="bg-white">
                                    <tr>
                                        <th
                                            class="w-10 px-4 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-400">
                                            No</th>
                                        <th
                                    ...
                                    @forelse ($penilaian as $item)
                                            ...
                                            @foreach ($surahsToDisplay as $loop2 => $surahHafalan)
                                                <tr class="transition-colors">
                                                    ...
                                                    <td class="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">
                                                        ...
                                                        <div class="text-xs text-gray-500">Ayat
                                                    ...
                                                    @if ($loop2 === 0)
                                                        <td class="px-4 py-3 text-sm capitalize"
                                                            rowspan="{{ $surahsToDisplay->count() }}">
                                                            <span
                                                                class="inline-flex px-2 py-1 text-xs font-semibold rounded-full {{ $item->jenis_penilaian_badge_tailwind }}">
                                                                {{ $item->jenis_penilaian_label }}
                                                            </span>
                                                        </td>
                                                        <td class="px-4 py-3 text-sm capitalize dark:text-gray-100"
                                                            rowspan="{{ $surahsToDisplay->count() }}">
                                                            {{ $item->jenis_hafalan }}
                                                        </td>
                                                        <td class="px-4 py-3 text-sm text-gray-900 dark:text-gray-100"
                                                        ...
                                                        <td class="px-4 py-3 text-sm font-bold text-center text-indigo-600"
                                                            rowspan="{{ $surahsToDisplay->count() }}">
                                                            {{ $item->nilai_total }}</td>
                                                        <td class="px-4 py-3 text-sm text-center"
                                                            ...
                                                            <span
                                                                ...
                                                                @if ($item->nilai_total >= 90) bg-green-100 text-green-600
                                                                @elseif($item->nilai_total >= 80) bg-indigo-100 text-indigo-600
                                                                @elseif($item->nilai_total >= 70) bg-yellow-100 text-yellow-600
                                        ...
                                        @else
                                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                                <td class="px-4 py-3 text-sm text-center text-gray-500">
                                                    {{ $rowNumber++ }}</td>
                                                <td class="px-4 py-3 text-sm italic text-gray-400">Surah tidak tersedia
                                                </td>
                                                <td class="px-4 py-3 text-sm capitalize">
                                                    <span
                                                        class="inline-flex px-2 py-1 text-xs font-semibold rounded-full {{ $item->jenis_penilaian_badge }}">
                                                        {{ $item->jenis_penilaian_label }}
                                                    </span>
                                                </td>
                                                <td class="px-4 py-3 text-sm capitalize">{{ $item->jenis_hafalan }}
                                                </td>
                                                <td class="px-4 py-3 text-sm">
                                                    {{ \Carbon\Carbon::parse($item->assessed_at)->translatedFormat('d M Y') }}
                                                </td>
                                                <td class="px-4 py-3 text-sm text-center">{{ $item->nilai_tajwid }}
                                                </td>
                                                <td class="px-4 py-3 text-sm text-center">{{ $item->nilai_harakat }}
                                                </td>
                                                <td class="px-4 py-3 text-sm text-center">{{ $item->nilai_makhraj }}
                                                </td>
                                                <td class="px-4 py-3 text-sm font-bold text-center text-indigo-600">
                                                    {{ $item->nilai_total }}</td>
                                                <td class="px-4 py-3 text-sm text-center">
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="10" class="px-4 py-12 text-center text-gray-500">
                                                ...
                                                <p class="font-medium text-gray-800">Tidak ada data penilaian pada periode ini</p>
                    ...
                    @if ($penilaian->isNotEmpty())
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                            ...
                            <div class="p-4 text-center bg-white shadow rounded-xl">
                                <div class="text-2xl font-bold text-indigo-600">{{ number_format($avgTotal, 1) }}
                                </div>
                            ...
                            <div class="p-4 text-center bg-white shadow rounded-xl">
                                <div class="text-2xl font-bold text-red-600">{{ $minTotal }}</div>
                ...
                @else
                    ...
                    <div class="p-8 text-center bg-white shadow-md rounded-xl dark:bg-gray-800">
                        ...
                        <h3 class="mb-2 text-lg font-medium text-gray-900 dark:text-white">Pilih Siswa untuk Melihat
                            Laporan</h3>
                        <p class="text-sm text-gray-500">Lengkapi semua filter di atas lalu klik <strong>Tampilkan
                                Laporan</strong></p>
                    </div>
                @endif
