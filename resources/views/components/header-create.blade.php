<div class="flex flex-col items-start justify-between gap-4 mb-6 sm:flex-row sm:items-center">
    <div>
        <div class="flex items-center gap-2 mb-1 text-sm text-[#8C8070]">
            <a href="{{ $route }}" class="hover:text-[#2D3F63]">{{ $breadcrumbTitle }}</a>
            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
            </svg>
            <span class="font-medium text-[#3A3028]">{{ $breadcrumbActive }}</span>
        </div>
        <h1 class="text-xl font-bold text-[#3A3028] md:text-2xl">{{ $title }}</h1>
        <p class="mt-1 text-sm text-[#8C8070]">{{ $description }}</p>
    </div>
    <a href="{{ $route }}"
